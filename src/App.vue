<template>
	<section class="bg-black flex h-screen text-white items-center justify-center select-none">
		<figure class="flex flex-col text-center w-11/12 gap-6 md:w-auto md:gap-8">
			<h1 class="font-primary font-bold text-5xl md:text-6xl">omercup</h1>
			<p class="flex font-secondary text-lg gap-4 items-start justify-center md:text-xl md:w-lg">
				<Icon
				...
				<a
					...
					v-bind:href="songLink ? songLink : 'https://open.spotify.com/track/57g9uWuZI1t822eLvEVQjn?si=0'"
...
useLanyard({
	...
	onPresenceUpdate(presence) {
		if (presence.listening_to_spotify) {
			showIcon.value = true;
			songLink.value = `https://open.spotify.com/track/${presence.spotify!.track_id}`;
			songName.value = `${presence.spotify!.artist} - ${presence.spotify!.song}`;
		} else {
			showIcon.value = false;
			songLink.value ='https://open.spotify.com/track/57g9uWuZI1t822eLvEVQjn?si=0';
